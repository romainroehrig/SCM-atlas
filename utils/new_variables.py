...
def f_zcb(zf,zneb):
    ...
    zcb = MV2.masked_values(zcb,cc.missing)
    zcb.missing_value = cc.missing
    zcb.id = 'zcb'
...
def f_zct(zf,zneb):
    ...
    zct = MV2.zeros(nt,typecode=MV2.float32) + cc.missing
    ...
    zct = MV2.masked_values(zct,cc.missing)
    zct.missing_value = cc.missing
    zct.id = 'zct'
...
def compute(filein,var):

    f = cdms2.open(filein,'a')
    if var == 'zcb':
        zf = f('zf')
        zneb = f('rneb')
        zcb = f_zcb(zf,zneb)
        ...
        print 'ERROR: variable to be computed is unknown:', var
        raise ValueError
